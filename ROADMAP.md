...
- [ ] **RS256 + JWKS + `aud` tek diyagramda** — 2026-09-25 uzun oturumunda "kim okur" (encoding) ve "kim üretir" (HS256 → RS256, JWKS, `kid`) satırları yapıldı. **Kalan yalnızca `aud` satırı** ("kimin için üretildi") + GitHub OIDC örneği; artık uzun oturum değil, 15 dakikalık bir tura sığar. Emre JWT'nin her gün dönmesinden yoruldu: bu tura en erken 2026-09-28'de, başka bir konu araya girdikten sonra dönülür. Eski not: `aud` kartı 3 kez `[zayıf]` kaldı (2026-09-19/22/23): "üretme yetkisi" ile "amaç dışı kullanım" hâlâ karışıyor. AWS Gün 1'deki GitHub OIDC akışı (imza + `aud: sts.amazonaws.com` kontrolü) örnek olarak kullanılır. Aşağıdaki RS256 [teori] maddesini de kapatır. Encoding/signature kartı da buraya bağlanır (2026-09-24: tanımlar doğru ama "payload base64 = herkes okur" uygulaması üçüncü kez kurulamadı).
...
- [x] **Refresh token** — [teori] ✅ 2026-09-25 günlük turda kapandı (kart: "Refresh token hangi problemi çözer?"): ömür ikilemi, sunucuda hash'li saklama, rotation + reuse detection, aile iptali. Uygulanmaz: production'da IdP işi.
- [x] **Cookie tabanlı authentication** — [teori] ✅ 2026-09-25'te refresh token maddesiyle birlikte kapandı. 2026-09-24'te istendi. **1. yarı yapıldı (2026-09-24):** session vs JWT, iptal, çoklu kopyada session, hibrit model (kart: "Session ile stateless JWT"). **2. yarı da yapıldı (2026-09-24):** localStorage vs `HttpOnly` cookie, `SameSite`, CSRF token, memory + refresh cookie kurulumu (kart `[zayıf]`: tek cümlelik trade-off). Kalan: refresh token maddesiyle birlikte pekiştirme. Sunucu tarafı session (`JSESSIONID`, Express'te `express-session`) ile header'daki Bearer JWT'nin karşılaştırması: state nerede tutuluyor, iptal etme ve yatay ölçekleme bedeli. Cookie bayrakları: `HttpOnly` (XSS'e karşı), `Secure`, `SameSite`. Cookie'ye geçince CSRF'in geri gelmesi (Faz 4'teki "token tabanlı API'da CSRF yapısal olarak yok" kartının tersi). Refresh token maddesiyle birlikte çalışılır, çünkü refresh token genelde `HttpOnly` cookie'de taşınır.
