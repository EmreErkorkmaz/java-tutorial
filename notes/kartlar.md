...
**Quiz protokolü:** her oturum başında buradan 3 soru — biri son fazdan, biri ortadan, biri eskiden. Bilemediğin kartın başına `[zayıf]` eklenir ve sonraki oturumlarda önce o sorulur; iki kez üst üste doğru cevaplayınca etiket düşer. **Aralık kuralı (2026-09-25):** etiket son sorulma tarihini taşır (`[zayıf · son: 2026-09-25]`); son sorulduğu günden itibaren **3 gün** dolmadan kart tekrar sorulmaz, yerine normal kartlar gelir. Bir quiz'de aynı konudan (ör. JWT/token) **en fazla bir** kart sorulur. Sebep: aynı kartları her gün sormak hem yoruyor hem de hatırlamayı değil kısa süreli belleği ölçüyor. `[zayıf]` yığılan bir konu, yol haritasında budadığımız yere geri dönme sinyalidir.
...
**S:** `[zayıf · son: 2026-09-25]` Encoding, hashing, encryption ve signature arasındaki fark? (2026-09-11'de doğru gelmişti, 2026-09-16'da HS256 sorusunda regresyon oldu — "JWT'yi secret ile şifreliyoruz" dendi, imza ≠ şifreleme ayrımı tekrar bulanıklaştı; 2026-09-19: "encryption çözülemez" dendi (o hashing), payload'a TC no sorusunda "payload herkesçe okunur, encoding ≠ encryption" gerekçesi kurulmadı; aynı oturumda tekrar turundan sonra dört kavram doğru kuruldu — 1/2, küçük kayma: imza "mühür bana mı ait" dendi, o `aud`'un işi. 2026-09-24 soğuk: dört tanım doğru (imza için "geri döndürülebilir: evet" dendi, kategori hatası — imza gizlemez, geri döndürülecek bir şey yok). TC sorusunda "koyamazsın" doğru, ama gerekçe "JWT bilgi taşımak için değil" ve "doğrulama TC ile yapılıyorsa konabilir" — ikisi de yanlış; payload = encoding bağlantısı yine kurulmadı. Sayılmadı, uzun oturumdaki RS256/aud diyagramına eklendi. 2026-09-25 uzun oturum: "herkes okur" doğru, gerekçe yine "token bilgi taşımak için değil" geldi (bizim token `roles`/`username` taşıyor); tek takip sorusundan sonra "base64 = encoding, anahtar gerekmez; encryption olsa okunamazdı" zinciri kuruldu. "Secret'ın içine rol gömülüyor mu?" sorusu çıktı: secret sabit bir araç, rol payload'da, imza ikisinin hash'i. Sayılmadı)
...
**S:** Session (cookie'de rastgele ID) ile stateless JWT arasındaki fark ne? Çalınmış bir hesabın oturumunu anında kapatabilir misin?
**C:** Fark **state'in nerede tutulduğu.** JWT'de state token'ın içindedir: sunucu yalnızca imza ve süreye bakar, DB'ye gitmez, her servis kendi doğrular. Bedeli şu: **anında iptal yoktur**, token kalan süresi kadar geçerlidir. Secret'ı değiştirmek herkesi çıkarır; tek kişiyi çıkarmak için bir kara liste gerekir, bu da sunucuyu yeniden stateful yapar ("iptal edebilmek için sunucunun hatırlaması gerekir"). Session'da cookie'de yalnızca anlamsız bir ID bulunur, bilgi sunucudaki tabloda durur. Her istekte arama yapılır; iptal etmek satırı silmekten ibarettir. Session kopyanın belleğinde tutulursa, birden fazla kopya varsa istekler **rastgele** 401 alır (container diskindeki dosya problemiyle aynı kök: state tek kopyada olursa kopyalar birbirinin yerine geçemez). Çözüm ortak depodur (Redis: Spring Session / `connect-redis`). Sticky session bir yamadır: kopya ölünce session'lar gider, yük eşit dağılmaz. Pratikteki cevap hibrittir: kısa ömürlü JWT access token + sunucuda kayıtlı, iptal edilebilir refresh token (`HttpOnly` cookie'de).
**Çapa:** JWT = mühürlü mektup (bilgi içinde, herkes okur, geri çağıramazsın). Session = vestiyer fişi (fişte yalnızca numara var, bilgi vestiyerin defterinde; iptal = satırı çizmek).
**Projede:** `product-service/.../config/SecurityConfig.java:47-54`: `SessionCreationPolicy.STATELESS`. Bu satır olmasaydı Spring kendiliğinden `JSESSIONID` üretirdi. 2026-09-24: "kılamam, secret değişirse herkes çıkar" soğuk soruda doğru geldi; çoklu kopya sorusunda cevap "Redis'te değilse doğrulanamaz" diye sıkıştırıldı, rastgelelik ve S3 bağlantısı söylenmedi.

**S:** `[zayıf · son: 2026-09-24]` Token localStorage'da mı tutulmalı, cookie'de mi? `HttpOnly`, `Secure` ve `SameSite` ne yapar, CSRF cookie ile neden geri gelir? (2026-09-24: parçalar tek tek doğru kuruldu, CORS gözlemi güçlüydü; ama tek cümlelik trade-off boşluk doldurma yardımıyla kurulabildi. Özet "JWT CSRF'e, cookie XSS'e karşı savunma" diye verildi — karşılaştırma JWT/cookie değil header/cookie, `HttpOnly` XSS'i değil yalnızca çalınmayı engeller)
**C:** **localStorage**: sayfada çalışan her script okur (kötü niyetli npm paketi, üçüncü parti script, XSS) ve token dışarı taşınır; refresh token da oradaysa saldırgan yeni token üretmeye devam eder. **`HttpOnly` cookie**: JS göremez, tarayıcı ilgili isteklere otomatik ekler. XSS'i zararsız yapmaz (script sekmenin içinden istek atabilir), yalnızca token'ın alınıp götürülmesini engeller. Otomatik ekleme **CSRF**'i geri getirir: `evil.com`'daki gizli form `shop.com`'a POST atar, cookie eklenir, sipariş oluşur. **CORS isteği engellemez, yalnızca cevabın okunmasını engeller**; düz form POST'unda preflight bile yoktur. Sunucu kimin istek atacağını engelleyemez; savunmanın hedefi yabancı sitenin isteğinin kimlik taşımaması ya da bir kanıt taşımıyorsa reddedilmesidir. **`SameSite`**: `Strict` = başka siteden hiçbir isteğe eklenmez; `Lax` (modern varsayılan) = yalnızca link tıklama/GET gezinmesine eklenir, POST/fetch'e eklenmez; `None` = her yere (`Secure` zorunlu), frontend ile API farklı sitelerdeyse gerekir ve CSRF koruması şarttır. **CSRF token**: sayfanın bildiği rastgele değer header'la geri yollanır; `evil.com` onu bilemez (same-origin policy sayfayı okumasını engeller). **`Secure`**: yalnızca HTTPS. Pratikteki kurulum: access token **memory'de** (JS değişkeni; axios interceptor header'a ekler, ömrü 15 dk), refresh token `HttpOnly; Secure; SameSite=Strict; Path=/auth/refresh` cookie'de. Sayfa yenilenince `/auth/refresh` çağrılır; `evil.com` bu isteği tetiklese bile cevaptaki token'ı okuyamaz. Tek cümle: header'da taşınan token CSRF'e bağışıktır ama XSS'le çalınabilir; `HttpOnly` cookie çalınmayı engeller ama CSRF'i geri getirir.
**Çapa:** Her seçenek bir saldırıya karşı korur, diğerine kapı açar. Hangi saldırı: token'ı kim ekliyor? Senin kodun (header) → CSRF yok; tarayıcı (cookie) → CSRF var.
**Projede:** `SecurityConfig.java:60` `csrf.disable()` yalnızca Bearer header kullanıldığı için güvenli; cookie'ye geçilen gün silinmeli (Spring Security'de CSRF koruması varsayılan olarak açık). Emre'nin React pratiği: localStorage + axios interceptor + refresh token.

**S:** Refresh token hangi problemi çözer? Neden 30 gün yaşayabilir de access token yaşayamaz? Çalınırsa sunucu bunu nasıl fark eder?
**C:** Tek token'da ömür ikilemi var: kısa ömür güvenli ama kullanıcı sık sık login olur; uzun ömür rahat ama çalınırsa ya da rol geri alınırsa token uzun süre geçerli kalır ve iptal edilemez. Çözüm iki token: **access** (JWT, 15 dk, her istekte her serviste, stateless) ve **refresh** (~30 gün, **yalnızca** `/auth/refresh`'e gider). Refresh uzun yaşayabilir, çünkü session gibi davranır: rastgele ve anlamsız bir değerdir, sunucuda saklanır (DB'de **SHA-256 hash'i**; BCrypt gerekmez, çünkü değer tahmin edilemeyen yüksek entropili bir sayıdır), iptal edilebilir ve az yerde görünür. Logout/iptal anında etkili olmaz: elde kalan access token en fazla 15 dk daha çalışır. **Rotation**: her refresh'te yeni bir refresh token verilir, eskisi "kullanıldı" diye işaretlenir. **Reuse detection**: kullanılmış bir token tekrar gelirse token kopyalanmış demektir. Sunucu iki kopyadan hangisinin saldırganda olduğunu bilemez, bu yüzden o login'in **token ailesinin tamamını** iptal eder: iki taraf da çıkar, gerçek kullanıcı parolasıyla geri döner, saldırgan dönemez. "Süresi dolmuş" normaldir, reddedilir; "kullanılmış" alarmdır. Tuzak: iki sekme aynı anda refresh ederse sahte alarm çıkar, birkaç saniyelik grace süresiyle çözülür.
**Çapa:** "Session ile stateless JWT" kartındaki hibrit: access = JWT tarafı, refresh = session tarafı. Rotation = tek kullanımlık bilet; aynı bilet iki kez gelirse biri kopyadır.
**Projede:** Yok. `TokenService.java:50` yalnızca 15 dk'lık access token basıyor, `AuthController` refresh token vermiyor; süre dolunca yeniden login gerekiyor. Production'da IdP işi (Cognito/Keycloak), teoride bırakıldı. 2026-09-25: üç fark (sunucuda saklanır, iptal edilebilir, yalnızca refresh endpoint'ine gider) ve rotation fikri kendiliğinden geldi. "Geçersiz token'la geleni logout et" dendi; eksik olan, sunucunun kimin saldırgan olduğunu bilemediği ve bu yüzden bütün aileyi iptal ettiğiydi.
...
**C:** Simetrik imzada **doğrulama yeteneği = üretme yeteneği**. Token'ı doğrulaması için sırrı verdiğin her servis token da basabilir; biri ele geçirilirse blast radius tüm sistem. Çözüm RS256 + JWKS: private key yalnızca üreticide, servisler public key ile sadece doğrular. HS256'da doğrulayan imzayı aynı secret'la **yeniden hesaplar** (bu yüzden basabilir); RS256'da yalnızca **kontrol eder**. Public key `/.well-known/jwks.json`'da herkese açık yayınlanır (sızması açık değildir); token header'ındaki `kid` hangi anahtarla doğrulanacağını söyler, bu da kesintisiz key rotation sağlar. (2026-09-25: order-service ele geçirilirse RS256'da yalnızca doğrulama, HS256'da basma — doğru geldi.)
...
**S:** `[zayıf · son: 2026-09-25]` "Token üretme yetkisi" ile "token'ın amacı dışında başka serviste kullanılması" aynı problem mi? (2026-09-22: üretme yetkisi doğru, ikinci problem hâlâ birinciyle aynı şey sanıldı — RS256'nın replay'i çözmediği ve `aud`'un görevi gelmedi. 2026-09-23: soğuk soruda yine "aynı problem" dendi, ama "token hangi servis için geçerli" fikri kendiliğinden geldi; yanlış olarak `aud`'un RS256'ya özgü olduğu sanıldı. "Kimse sahteleme yapmıyor" senaryosundan sonra doğru kuruldu — sayılmadı. Üçüncü takılma: yeni senaryo çapanın yanına eklendi. 2026-09-24 soğuk soruda doğru — 1/2: "amaç dışı kullanım, imza alıcıyı kontrol etmez, `aud` engeller". Eksik nüans: `aud` yanlış kapıyı kapatır, çalınmış token'ı değil; aynı token asıl hedefinde (product-service) hâlâ geçerli, ona karşı savunma kısa ömür. 2026-09-25 soğuk: "aynı problem değil" ve kısa ömür nüansı doğru geldi, ama "ne engeller?" sorusuna "başka servisler token üretemez" dendi — birinci problemin cevabı, dördüncü kez birleşme. `aud` ancak takip sorusundan sonra geldi, "token'ın payment'a erişimi yok" diye yetki gibi anlatıldı; `aud` yetki değil adres. Sayılmadı, seri sıfırlandı)
